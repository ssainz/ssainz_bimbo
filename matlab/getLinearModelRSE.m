function [rse, models, data_matrix, rmsle_mat, test_matrix] = getLinearModelRSE()
...
for siz = sizes
    data = getSampleData(500, siz{1}, 10000);
    ...
    j = 1;
    for vv = bimbovars 
        mdl = fitlm(data_table, 'quadratic', 'ResponseVar', 'venta', 'PredictorVars', vv, 'CategoricalVars', vv);
        rse(i, j) = mdl.RMSE;
...
rse = table(rse(:,1), rse(:,2), rse(:,3), rse(:,4), rse(:,5), rse(:,6), 'VariableNames', {'agencia' 'canal' 'ruta' 'cliente' 'producto' 'samana'}, 'RowNames',sizeChar);
